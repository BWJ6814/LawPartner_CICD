...
def add_more_data_to_db(segment: int) -> None:
    ...
    print(f"[DOWNLOAD] 세그먼트 {segment}: {start}~{end}건 다운로드 중...", flush=True)
    ds = load_dataset(
        "parquet",
    ...
    print("[SPLIT] 텍스트 분할 중...", flush=True)
    ...
    for bi, i in enumerate(range(0, n, batch_size), start=1):
        ...
        try:
            ...
            print(f"[OK] 배치 {bi}/{total_batches} 완료 및 저장!", flush=True)
    ...
    print(f"[DONE] 세그먼트 {segment} 완료! ({n}개 저장, DB: {DB_PATH})", flush=True)
