...
# 1. 환경 변수(.env)에서 구글 API 키를 안전하게 불러옵니다. (키 값은 절대 로그에 남기지 않음)
load_dotenv()
...
async def chat(request: QueryRequest):
    """async + threadpool: Uvicorn이 응답을 안정적으로 마무리하고, 블로킹은 스레드에서 처리."""
    logger.info("POST /chat received (question len=%s, disable_rag=%s)", len(request.question or ""), request.disable_rag)
    try:
        payload = await run_in_threadpool(_chat_sync, request)
        # Content-Length가 명확한 JSON 바이트로 응답 (Java HttpURLConnection 연결 끊김 완화)
...
def summarize_consult(request: SummarizeConsultRequest):
    ...
    try:
        ...
        logger.exception("summarize-consult 처리 중 오류: %s", e)
        raise HTTPException(status_code=500, detail=str(e))