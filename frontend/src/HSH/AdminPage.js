...
export default function AdminPage() {
  ...
  // 6. 보안 정책 설정 (금지어 관리 - TB_BANNED_WORD 연동)
      ...
      <div className="space-y-6 animate-in fade-in duration-500">
        <Card title="시스템 금지어 필터링 관리" rightElement={<ShieldAlert className="text-rose-500" size={20} />}>
          ...
          <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
            <h4 className="text-xs font-black text-slate-400 mb-4 uppercase tracking-widest">현재 등록된 금지어 목록 ({bannedWords.length}개)</h4>
  ...
  // 7. 콘텐츠 보안 관리 (게시물 블라인드 - TB_BOARD 연동)
  const ContentSecurityView = () => (
